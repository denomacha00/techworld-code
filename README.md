...
- **Safe autonomy** — a command-safety policy always requires a human click for dangerous commands (`rm -rf`, disk format, force push, `curl | sh`, `sudo`, …) **even with auto-approve on**; add your own via `techwordCode.blockedCommands`. Configurable step budget for long/overnight tasks, auto-retry on transient failures, never hangs silently, Stop halts work (including running commands) instantly.
